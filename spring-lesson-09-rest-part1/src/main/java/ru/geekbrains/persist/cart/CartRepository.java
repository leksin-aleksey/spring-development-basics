...
public class CartRepository {
    ...
    public CartRepository(CustomerRepository customerRepository, ItemRepository itemRepository) {
        ...
        itemsIds.add(itemRepository.findById(1L).orElseThrow(NotFoundException::new));
        itemsIds.add(itemRepository.findById(2L).orElseThrow(NotFoundException::new));
        cart.setItems(itemsIds);
        carts.put(2L, cart);

        cart = new Cart(customerRepository.getOne(3L));
        itemsIds = new ArrayList<>();
        itemsIds.add(itemRepository.findById(3L).orElseThrow(NotFoundException::new));
        itemsIds.add(itemRepository.findById(4L).orElseThrow(NotFoundException::new));
        itemsIds.add(itemRepository.findById(5L).orElseThrow(NotFoundException::new));
        cart.setItems(itemsIds);
        carts.put(2L, cart);
    }
